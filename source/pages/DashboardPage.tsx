...
/* ============================ 商机漏斗（数量 / 金额双口径） ============================ */
// 口径：全部由商机主数据派生，阶段取 OPP_STAGES 真实 5 档（未中标 / 关闭为终态，不入漏斗）；
// 数量口径 = 商机个数；金额口径 = 加权（金额 × 阶段概率）。禁止硬编码档位数值。
const TERMINAL = OPP_TERMINAL as readonly string[];
/** 漏斗行：数量档 value = 个数；金额档 value = 加权额（元，用于条长比例） */
function buildFunnel(mode: 'c' | 'a') {
  const live = OPPS.filter((o) => !TERMINAL.includes(o.stage));
  const rows = OPP_STAGES.map((stage) => {
    const list = live.filter((o) => o.stage === stage);
    const prob = OPP_STAGE_PROB[stage] ?? 0;
  ...
  return { rows, live, total: live.reduce((s, o) => s + o.amt, 0), weighted: Math.round(live.reduce((s, o) => s + o.amt * ((OPP_STAGE_PROB[o.stage] ?? 0) / 100), 0)) };
}
...
/* ============================ 岗位视角提示（tip / warn 两种） ============================ */
const TIPS: Record<string, { tone: 'tip' | 'warn'; text: React.ReactNode }> = {
  deputy: { tone: 'tip', text: <><Ico n="user" size={16} /> 分管负责人：数据范围 = <b>分管团队</b>；资金 / 盈亏指标限分管项目；「待我审批」= 分管节点 + 本人节点。</> },
  sales: { tone: 'tip', text: <><Ico n="wallet" size={16} /> 销售人员：聚焦<b>客户与商机推进</b>；公司级金额指标按 A-02 显示「—」；本角色不可见公司资金 / 项目盈亏 / 证书预警 / 系统健康。</> },
  pm: { tone: 'tip', text: <><Ico n="wrench" size={16} /> 施工人员：聚焦<b>项目执行与验收闭环</b>；金额类不可见（A-02），本页聚焦任务 / 进度 / 整改。</> },
  finance: { tone: 'tip', text: <><Ico n="wallet" size={16} /> 财务：聚焦<b>资金收付与账龄核销</b>；红字冲销 = 负数单冲正且不回退合同状态，仅财务 + 管理员有权限。</> },
  admin: { tone: 'warn', text: <><Ico n="shield" size={16} /> 行政角色（系统管理员 + 证书管理员）<b>不涉经营金额</b>：全部金额类指标按 A-02 显示「—」；本页聚焦用户 / 证书 / 系统健康 / 待办引导。</> },
  sysadmin: { tone: 'warn', text: <><Ico n="gear" size={16} /> 超级管理员：可维护枚举 / 模板 / 提醒矩阵 / 租户 Logo；经营金额类指标按 A-02 脱敏显示，用于验证权限裁剪是否生效。</> },
};
...
export default function DashboardPage({ go, role, nav }: { go: (p: string) => void; role: string; nav?: number }) {
  ...
  const overdueRecv = RECEIVABLES.filter((r) => r.status === '逾期');
  ...
  const certNeedInvoices = CONTRACTS.filter((c) => c.status === '履约中' && c.recvPct < 50);
  ...
  const myOpps = OPPS.filter((o) => !TERMINAL.includes(o.stage)).sort((a, b) => b.amt - a.amt);
  ...
  const dueIn = (days: number) => RECEIVABLES
    .filter((r) => r.status !== '逾期' && (() => { const d = daysFromToday(r.dueDate); return d >= 0 && d <= days; })())
  ...
  const blockMoney = (title: string, opts?: { todoAppr?: number; hint?: React.ReactNode }) => {
    const o = opts ?? {};
    return (
      <Card hd={<>{title}</>} extra={<span className="nc-cell-sub">口径见 hover 提示</span>}>
        ...
        <div className="nc-dash-hint">
          范围切换仅作用<b>区间类</b>（本月应收 · 本年新增客户 · 商机加权 · 成本发生额）；
          <b>时点类</b>（逾期应收 · 待我审批 · 待付款 · 保证金未退）<b>不随动</b>。
          {o.hint}
        </div>
      </Card>
    );
  ...
  /** 商机漏斗 */
  const blockFunnel = (title: string, extraHint?: string) => (
    <Card
      hd={title}
      extra={
    ...
    >
      ...
      <div className="nc-dash-funnel-foot">
        <span className="nc-cell-sub">
          加权 = 金额 × 概率 ｜ 在谈总额 {money ? fmtWan(funnel.total) : '—'} · 加权 {money ? fmtWan(funnel.weighted) : '—'}
        </span>
        <Btn size="sm" onClick={() => go('opp')}>穿透商机列表 →</Btn>
      </div>
      <div className="nc-dash-hint">穿透 → 商机列表 ｜ 漏斗阶段档位可配置{extraHint}</div>
    </Card>
  );

  /** 投标在途 —— 6 阶段计数（子集：报名 / 招标中 / 做标书 / 已交保证金 / 已投标 / 开标待登记） */
  const blockBid = () => {
    const stages: { label: string; real: string }[] = [
      { label: '报名', real: '报名' },
      { label: '招标中', real: '招标中' },
      { label: '做标书', real: '做标书' },
      { label: '已交保证金', real: '已交保证金' },
      { label: '已投标', real: '已投标' },
      { label: '开标待登记', real: '开标' },
    ];
    return (
      <Card hd={<><Ico n="mail" size={16} /> 投标在途</>} extra={<Btn size="sm" onClick={() => go('bid')}>投标管理 →</Btn>}>
        ...
        <div className="nc-dash-hint">
          阶段口径：8 阶段中前 6 阶段为「在途」；「开标」= 已投标待登记结果；
          <b>安全生产许可证过期 = 全部投标废标</b>，投标引用前须校验证书三要素。
        </div>
      </Card>
    );
  };
  ...
  const blockCert = () => (
    <Card hd={<><Ico n="scroll" size={16} /> 证书预警</>} extra={<Btn size="sm" onClick={() => go('cert')}>证书管理 →</Btn>}>
      ...
      <div className="nc-dash-hint">
        履约期过期：<b>占用维持 + 提醒持证人 / 项目经理 + 不影响验收</b>（N-61 口径）；
        过期证书禁止借给项目 / 投标引用（{safety ? `${safety.name} 有效期至 ${safety.validTo}` : '安许'}须在临期前完成续证安排）。
      </div>
    </Card>
  );
  ...
  const blockCustomer = (cols = 2) => (
    <Card hd={<><Ico n="users" size={16} /> 客户卡</>} extra={<Btn size="sm" onClick={() => go('customer')}>客户管理 →</Btn>}>
      ...
      <div className="nc-dash-hint">
        客户建档完整度 ≥95% ｜ 拜访必上传照片（水印 + GPS），跟进记录提交后不可改（4.2）｜
  ...
  const blockApproval = (rows: typeof APPROVALS, total: number) => (
    ...
    >
      ...
      <div className="nc-dash-hint">
        待批统计：本月 0 笔（待批开关默认关闭）｜ <b>无合同付款固定路由总经理</b>（决策 21/24，单点签收留痕）。
      </div>
    </Card>
  );
  ...
  const blockTodo = () => (
    <Card hd="我的待办" extra={<><Tag tone="blue">个人提醒直达操作 · 站内 + 钉钉</Tag><Btn size="sm" onClick={() => { setRead([]); toast('待办已全部标记为已读'); }}>全部标记已读</Btn></>}>
      ...
      <div className="nc-dash-hint">提醒频控 ≤5 条/人日 ｜ 提醒事件 = 角色 × 渠道矩阵（4.9）｜ 处理完自动销提醒。</div>
    </Card>
  );
  ...
  const blockMyOpp = (title: string, scopeTag: string) => (
    <Card hd={title} extra={<Tag tone="blue">{scopeTag}</Tag>}>
        ...
        <tbody>
          {myOpps.slice(0, 3).map((o) => (
            <tr key={o.id}>
              ...
              <td>
                <Tag tone={o.stage === '商务谈判' ? 'gold' : o.stage === '甲方立项确认' ? 'orange' : 'gray'}>
                  {o.stage}{o.amt === 0 ? ' ⊙未填' : ''}
                </Tag>
              </td>
      ...
      <div className="nc-dash-hint">阶段 ≥「发包方立项确认」时预计金额必填，可就地补填 ｜ 关联投标结果自动回写商机阶段。</div>
    </Card>
  );
          ...
          <tbody>
            {mine.map((b) => (
              <tr key={b.id}>
                ...
                <td>
                  <Tag tone={b.stage === '未中标' ? 'red' : b.stage === '中标' ? 'green' : 'blue'}>{b.stage}</Tag>
  ...
  const myProjectKpiBody = () => (
    <>
      <div className="nc-kpi-grid is-6">
        <Kpi label="在建项目" value={PROJECTS.filter((p) => p.status === '执行中').length} tone="blue" sub="本人负责施工" onClick={() => go('project')} drill />
      ...
      <div className="nc-dash-hint">
        报工须上传现场照片（水印 + GPS）｜ 整改闭环后自动通知项目经理验收；
        金额类指标按 A-02 对施工角色不可见。
      </div>
    </>
  );
  ...
  const blockOverdueRecv = () => (
    <Card hd="收款逾期期次" extra={<span className="nc-cell-sub">行内直达操作（与合同详情双向同源）</span>}>
      <table className="nc-tbl is-md">
        ...
        <tbody>
          {overdueRecv.map((r) => (
            <tr key={r.id} className="nc-row-warn">
              ...
              <td><Tag tone="red">逾期 {r.overdueDays} 天</Tag></td>
              <td className="is-num">
      ...
      <div className="nc-dash-hint">
        逾期 <b>7 / 30 / 60</b> 天分级提醒（去重随提醒重置发）｜ 登记后合同累计已收 / 回款率 / 驾驶舱当日刷新。
      </div>
    </Card>
  );
  ...
  const blockPayable = () => (
    <Card hd="付款待处理" extra={<span className="nc-cell-sub">含无合同付款徽标 · 特批徽标</span>}>
      ...
      <div className="nc-dash-hint">
        付款校验：<b>累计已付 + 本次 ≤ 执行金额 × 付款比例上限</b>（默认 100%，可配）；
        红字冲销 = 负数单冲正、<b>不回退合同状态</b>，仅财务 + 管理员可操作。
      </div>
    </Card>
  );
  ...
  const cashflowBody = (withQuick: boolean) => (
    <>
      ...
      <div className="nc-dash-hint">
        预收款 &gt;30 天未归并自动触发待办提醒；归并向导 = 项目经理 + 管理员执行（24 小时内可撤销）。
        成本发生额（{sc.label}）{money ? sc.cost : '—'} 属<b>权责口径</b>，与本卡现金流口径不等价。
      </div>
      {withQuick && (
        <div className="nc-quickbar">
          ...
          <button className="nc-qk" onClick={() => go('project-center')}><Ico n="book" size={16} /> 成本台账</button>
        </div>
      )}
    </>
  );
          ...
          <div style={{ marginTop: 12 }}>
            {bizTab === 'profit' ? (
              <>
                ...
                <div className="nc-dash-hint">
                  亏损项目 = 实际毛利率 &lt; 0；成本超支 = 实际成本 &gt; 计划成本（联动变更单）。两个口径<b>不等价</b>：权责看已发生成本，现金流只看已收付。
                </div>
                {profitTail()}
              </>
  ...
  const renderView = () => {
    ...
    if (view === 'deputy') {
      return (
        <>
          <div className="nc-dash-row">
            <div className="nc-dash-main">{blockMoney(' 分管资金与合同', { hint: <>分管口径：数据范围 = <b>分管团队</b>，资金 / 盈亏限分管项目（演示租户未拆分团队，金额与全量同源）。</> })}</div>
            {blockFunnel(' 团队漏斗')}
          </div>
          <div className="nc-dash-2">
            {blockAlert([
      ...
      return (
        <>
          <div className="nc-dash-row">
            <div className="nc-dash-main">
              <Card hd="我的客户" extra={<Tag tone="blue">本人数据范围</Tag>}>
                ...
                <div className="nc-dash-hint">
                  拜访必上传照片（水印 + GPS）· 提交后不可改（4.2）｜ 公司金额显示「—」= <b>脱敏而非隐藏</b>口径。
                </div>
              </Card>
            </div>
            {blockFunnel(' 我的商机漏斗', '（个人口径）')}
          </div>
          <div className="nc-dash-2">
          ...
          <Card hd="快捷操作">
            ...
            <div className="nc-dash-hint">新建 / 跟进后自动回流：漏斗阶段、待办提醒即时刷新。</div>
          </Card>
        </>
      );
    }
        ...
        <>
          <div className="nc-dash-2">
            ...
            <Card hd={<><Ico n="bell" size={16} /> 施工提醒</>} extra={<Tag tone="gray">频控 ≤5 条/人日</Tag>}>
              ...
              <Alert icon={<Ico n="package" size={16} />} title="材料到货确认：辅材 1 批（关联 PF20260918-0009）" sub="确认后同步采购收货状态" op={<Op onClick={() => toast('已确认到货 · 同步采购收货状态（演示）')}>确认到货</Op>} />
            </Card>
          </div>
          <div className="nc-dash-2">
            {blockTodo()}
        ...
        <>
          ...
          <div className="nc-dash-2">
            <Card hd="发票与税额" extra={<Btn size="sm" onClick={() => go('invoice')}>发票管理 →</Btn>}>
              ...
              <div className="nc-dash-hint">
                税率口径：<b>含税 税额 = 总额 × 税率 ÷ (100 + 税率)</b>；不含税 <b>÷ 100</b>。
                {(() => {
                  const inv0 = INVOICES[0];
                  const t = calcTax(inv0.amt, inv0.taxRate, '含税');
                  return <>示例：{inv0.id} 不含税 {fmt(inv0.amt)} × {inv0.taxRate}% → 税额 {fmt(t)}、含税 {fmt(inv0.amt + t)}。</>;
                })()}
                开票税率须与合同税率一致，不一致为硬拦截。
              </div>
            </Card>
            <Card hd="合同履约与开票" extra={<Btn size="sm" onClick={() => go('contract')}>合同管理 →</Btn>}>
                ...
                <tbody>
                  {CONTRACTS.slice(0, 4).map((c) => (
                    <tr key={c.id} className={c.overdue ? 'nc-row-warn' : undefined}>
                      <td><EntityLink target="contract" id={c.id} go={go} title="下钻到合同详情">{c.id}</EntityLink></td>
                      <td><Tag tone={c.status === '履约中' ? 'blue' : normContractStatus(c.status) === '审批中' ? 'orange' : 'gray'}>{c.status}</Tag></td>
              ...
              <div className="nc-dash-hint">{certNeedInvoices.length} 份履约中合同回款率 &lt;50%，须关注账龄与开票节奏。</div>
            </Card>
          </div>
        </>
      );
      ...
      <>
        <div className="nc-dash-2">
          <Card hd="用户" extra={<span className="nc-cell-sub">离职清零方可停用（H-34 主入口）</span>}>
            ...
            <div className="nc-dash-hint">账号 = 手机号唯一 ｜ 停用时立即失效会话 ｜ 离职须先完成归属项清零。</div>
          </Card>
          {blockCert()}
        </div>
        <div className="nc-dash-2">
          <Card hd="待办 · 离职清零引导" extra={<Tag tone="orange">逐项处理后方可停用</Tag>}>
            ...
            <div className="nc-dash-hint">
              未清零时「确认停用」被拦截（H-34）：证书外借必须先<b>收回外借</b>，借给项目必须先<b>用完收回</b>。
            </div>
          </Card>
        ...
        <div className="nc-dash-2">
          <Card hd="供应商准入" extra={<Btn size="sm" onClick={() => go('supplier')}>供应商管理 →</Btn>}>
            ...
            <div className="nc-dash-hint">黑名单供应商在合同 / 付款 / 询价环节一律硬拦截；准入证照到期未更新自动冻结。</div>
          </Card>
          <Card hd="行政快捷入口">
            <div className="nc-quickbar" style={{ marginTop: 0, borderTop: 'none', paddingTop: 0 }}>
  ...
  return (
    <>
      ...
      <div className="nc-rolebar">
        ...
        <span className="nc-rolebar-hint">同一租户同一数据源，仅按「角色 × 范围 × 权限层级」裁剪视图</span>
      </div>

      {/* 岗位口径提示 */}
      {tip && <div className={tip.tone === 'tip' ? 'nc-tipline' : 'nc-warnline'}>{tip.text}</div>}
      ...
      {/* 常驻口径说明 */}
      <Card hd="口径说明（常驻）">
        <div className="nc-caliber">
          <Ico n="pin" size={14} /> 所有指标由基础数据自动推算，<b>禁止人工填报</b>；口径说明 Tooltip + 卡片 note 双通道常驻。
          数据范围仅作用<b>区间类</b>指标（本月 / 本季 / 本年应收 · 回款预测 · 本年新增客户 · 商机加权 · 成本发生额）；
          <b>时点类</b>指标（逾期应收 · 待我审批 · 待付款 · 保证金未退）不随动。
          金额类按 A-02 权限裁剪：无权限显示「—」脱敏而非隐藏。穿透 ≤3 click；
          导出自动加水印（导出人 + 时间 + 租户）。
        </div>
      </Card>
    </>
