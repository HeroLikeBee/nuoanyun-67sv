  ...
  return (
    <>
      <PageHead
        ...
        actions={<>
          <Btn onClick={() => setWmOpen(true)}>导出台账</Btn>
      ...
      */}
      ...
      <WatermarkModal
        open={wmOpen}
        scope={`证书台账（在册 ${certs.length} 本）`}
        onClose={() => setWmOpen(false)}
        onConfirm={(cfg) => { setWmOpen(false); toast(`已导出台账（水印：${cfg.text}）· 含在册 ${certs.length} 本证书`); }}
      />
    </>
  );
}
