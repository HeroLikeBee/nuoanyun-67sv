...
/* G3：原 const Q = QUOTES[0] 硬编码索引 0 —— 从任何入口进入都只看 BJ20260912-0011。
...
export default function QuoteDetailPage({ go, role, nav }: { go: (p: string) => void; role: string; nav?: number }) {
  ...
  /** 明细随报价单联动：总额锚定 Q.total（可溯源），区域上浮在其上生效 */
  const LINES = useMemo(() => linesOf(Q.total), [Q]);
  ...
  const devText = Math.abs(dev) <= 10
    ? `当前单方 ${unit.toFixed(1)} 元/㎡，同类历史均值 ${mean.toFixed(1)} 元/㎡，偏离 ${dev > 0 ? '+' : ''}${dev.toFixed(1)}%，处于合理区间（±10%）。`
    : `当前单方偏离历史均值 ${dev > 0 ? '+' : ''}${dev.toFixed(1)}%（${Math.abs(dev) > 20 ? '超出 ±20%，需重点说明定价依据' : '±10%~20%，建议复核'}）。`;
    ...
    <>
      ...
      <div className="nc-tiles nc-tiles-4">
        <div className="nc-tile">
          ...
          <div className="nc-tile-sub">{t.mode === '含税' ? '含税口径：总额即客户应付' : '不含税口径：开票另加税额'}</div>
        </div>
        <div className="nc-tile">
          <div className="nc-tile-label">税额</div>
          <div className="nc-tile-value num"><Money v={t.tax} role={role} /></div>
          <div className="nc-tile-sub">{t.mode} {t.rate}% · 不含税 <Money v={t.net} role={role} /> / 含税合计 <Money v={t.gross} role={role} /></div>
        </div>
        <div className="nc-tile">
          <div className="nc-tile-label">区域上浮</div>
          <div className="nc-tile-value num" style={{ color: 'var(--c-primary)' }}>+{uplift}%</div>
          <div className="nc-tile-sub">云南区域默认上浮 3%</div>
        </div>
        <div className="nc-tile">
          <div className="nc-tile-label">工程费单方（不含设备）</div>
          <div className="nc-tile-value num">{unit.toFixed(1)} 元/㎡</div>
          <div className="nc-tile-sub">工程费 {fmt(eng)} ÷ {AREA.toLocaleString('en-US')} ㎡</div>
        </div>
      </div>
          ...
          <table className="nc-tbl" style={{ minWidth: 860 }}>
            ...
            <tfoot>
              <tr className="nc-tbl-sum">
                <td colSpan={4}>合计 <span className="nc-cell-sub">金额 = 数量 × 单价 × (1 + 区域上浮)，自动计算</span></td>
      ...
      <Card hd="历史同类项目价格参照" extra={<span className="nc-cell-sub">行业「{Q.base}」· 类型「消防系统升级」· 近 18 个月成交</span>}>
        ...
        <div className={`nc-warnbox ${devTone}`}>
          ...
          <div>{devText}区间口径：偏离均值 ±10% 内绿 · ±20% 内黄 · &gt;±20% 红（供审核参考，不作为拦截依据）。</div>
        </div>
      </Card>
      ...
      <Card hd="版本记录" extra={<span className="nc-cell-sub">多轮报价逐版留痕，可对比追溯</span>}>
        <div className="nc-vercard is-cur">
          ...
          <b className="num"><Money v={total} role={role} /></b>
        ...
        <div className="nc-vercard">
          <Tag>V1</Tag>
          <span className="num">{fmt(4862000)}</span>
      ...
      {/* 生成 V3（必填变更原因） */}
      <Modal
        open={v3Open} onClose={() => setV3Open(false)} width={520} title="生成 V3"
      ...
      {/* 撤回审批 */}
      <Modal
        open={recallOpen} onClose={() => setRecallOpen(false)} width={440} title="撤回审批"
      ...
      {/* 版本差异回放（含性质列） */}
      <Modal open={diffOpen} onClose={() => setDiffOpen(false)} width={760} title="V1 → V2 变更对比"
