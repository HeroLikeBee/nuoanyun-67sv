...
import { getContracts, consumeFocus, subscribeStore, getBizStatus } from '../components/store';

const ST_TONE: Record<string, 'gray' | 'blue' | 'green' | 'red' | 'orange' | 'purple'> = {
  草稿: 'gray', 待审批: 'orange', 审批中: 'orange', 已审批: 'blue', 已签约: 'green',
  履约中: 'blue', 结算中: 'orange', 已结项: 'gray', 已续签: 'purple', 已终止: 'red',
  已中止: 'red', 已解除: 'red',
};
...
const TABS = ['全部', '待我审批', '待签约', '收款逾期', '已结项', '已续签', '已终止'] as const;
...
export default function ContractPage({ go, role, nav }: { go: (p: string) => void; role: string; nav?: number }) {
  ...
  const counts: Record<string, number> = {
    ...
    待签约: contracts.filter((c) => st(c) === '已审批').length,
  ...
  const rows = useMemo(() => contracts.filter((c) => {
    ...
    if (tab === '待签约' && st(c) !== '已审批') return false;
