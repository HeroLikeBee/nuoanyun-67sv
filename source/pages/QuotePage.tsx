...
export default function QuotePage({ go, role, nav }: { go: (p: string) => void; role: string; nav?: number }) {
  ...
  const cols = [
    ...
    { key: 'customer', title: '客户', width: 200, render: (q: Q) => <EntityLink target="customer" id={q.customerId} go={go} title="下钻到客户档案">{q.customer}</EntityLink> },
    { key: 'opp', title: '关联商机', width: 110, render: (q: Q) => (q.opp ? <EntityLink target="opp" id={q.opp} go={go} title="下钻到商机详情">{q.opp}</EntityLink> : <span className="nc-cell-sub">—</span>) },
  ...
  return (
    <>
      ...
      <Drawer
        ...
        foot={detail && (
          <div className="nc-ops">
            ...
            {detail.status === '已审批' && <Btn kind="primary" size="sm" onClick={() => { setDetail(null); go('contract-new'); }}>转合同（原子创建项目 + 合同草稿）</Btn>}
            <Btn size="sm" onClick={() => setVerOpen(detail)}>版本管理</Btn>
      ...
      {/* ============ 新建报价单 ============ */}
      <Modal
        open={newOpen} onClose={() => setNewOpen(false)} width={680} title="新建报价单"
      ...
      {/* ============ 材料库·价格档案 ============ */}
      <Drawer open={libOpen} onClose={() => setLibOpen(false)} width={720} title="材料库 · 价格档案" sub="报价的唯一成本价来源（成本参考价锁死，改动留痕）">
      ...
      <Drawer open={integOpen} onClose={() => setIntegOpen(false)} width={560} title="穿透设置" sub="报价 ↔ 商机 ↔ 合同的数据联动规则">
        ...
        <KvGrid cols={1} rows={[
          { k: '报价 → 商机', v: '报价单挂载关联商机；已审批/已转化回写商机加权金额与阶段（→ 已报价）' },
          { k: '报价 → 合同', v: '已审批点「转合同」→ 原子创建项目（来源=报价转化）+ 合同草稿，幂等防重' },
      ...
      {/* ============ 提交审批 ============ */}
      <Modal
        open={!!submitOpen} onClose={() => setSubmitOpen(null)} width={600} title={`提交审批 · ${submitOpen?.id ?? ''}`}
      ...
      {/* ============ 版本管理 ============ */}
      <Drawer open={!!verOpen} onClose={() => setVerOpen(null)} width={620} title="版本管理" sub={verOpen && `${verOpen.name} · 当前 ${verOpen.ver}`}>
        {verOpen && (
          <>
      ...
      {/* ============ 版本差异对比 ============ */}
      <Modal open={diffOpen} onClose={() => setDiffOpen(false)} width={820} title="V1 → V2 变更对比"
