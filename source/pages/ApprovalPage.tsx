...
import {
  Btn, Card, DataTable, Field, ListToolbar, Modal, Money, Op, OpSep,
...
export default function ApprovalPage({ go, role, nav }: { go: (p: string) => void; role: string; nav?: number }) {
  ...
  const [types, setTypes] = useState<string[]>(['报价审批', '合同审批', '付款申请', '变更审批']);
    ...
    <>
      ...
      <Card flush>
        ...
        <DataTable
          ...
          emptyCta={<Btn size="sm" onClick={() => { setTypes(['报价审批', '合同审批', '付款申请', '变更审批']); setStF('all'); setLv('all'); setKw(''); }}>清空筛选条件</Btn>}
      ...
      {/* ============ 详情抽屉 ============ */}
      {detail && (
        <Modal open title={`审批详情 · ${detail.id}`} width={760} onClose={() => setDetail(null)}
          ...
          <Field label="审批链（分级自动路由）" span={4}>
            ...
            <div className="nc-tiny nc-muted" style={{ marginTop: 8 }}>
              路由规则：&lt;50 万 → 部门负责人；50~200 万 → 分管副总；≥200 万 → 总经理（采购类按 30/100 万口径，维护保养类按 100 万口径）；合同 / 报价额外追加财务复核节点。
            </div>
          </Field>
          ...
          {detail.type === '报价审批' && (() => {
            ...
            return (
              <Field label="历史同类项目价格参照" span={4}>
                <div className="nc-tiny nc-muted" style={{ marginBottom: 8 }}>审核不再凭经验——系统已自动匹配 {REF_HIST.length} 单同类成交</div>
                {REF_HIST.map((r) => (
      ...
      {/* ============ 退回 ============ */}
      <Modal open={!!reject} title={`退回审批 · ${reject?.id || ''}`} width={520} onClose={() => setReject(null)}
      ...
      {/* ============ 评审 B2：已退回 → 重新提交 ============ */}
      <Modal open={!!resubmit} title={`重新提交审批 · ${resubmit?.id || ''}`} width={560} onClose={() => setResubmit(null)}
