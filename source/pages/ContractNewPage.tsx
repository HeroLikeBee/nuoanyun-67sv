...
const PROJ_OPTS = PROJECTS.filter((p) => p.status !== '已结项').map((p) => ({
  id: p.id, name: p.name, a04: p.id === 'XM000087',
}));

/** 复制历史：仅 已签约 / 履约中 / 已结项；本客户优先 → 同类型 */
const HIST = [
  {
    id: 'HT20260912-0009', name: '昆明万达广场消防改造工程合同', type: '销售合同',
  ...
  {
    id: 'HT20260818-0005', name: '丽江景区智慧消防平台合同', type: '销售合同',
  ...
  {
    id: 'WB20260901-0003', name: '楚雄州人民医院消防维护保养合同（2027）', type: '维护保养合同',
...
export default function ContractNewPage({ go, role, nav }: { go: (p: string) => void; role: string; nav?: number }) {
  ...
  const [route, setRoute] = useState<number[]>([0, 3]);
  ...
  if (pmode === 'draft' && f.pjname.trim()) issues.push(['proj', `草稿项目「${f.pjname.trim()}」签约后自动转已立项，请确认名称`]);
  ...
  const nextLabel = step === 0 ? '下一步'
    : step === 1 ? ({ ocr: '进入 Step2 校对 →', std: '生成草稿，进入 Step3', ent: '生成草稿，进入 Step3', copy: '确认带入，进入 Step3' } as Record<string, string>)[src ?? ''] ?? '下一步'
      : step === 2 ? '确认结果，进入 Step3' : '提交审批（分级快照）';
  ...
  const runOcr = () => {
    ...
    const t = setInterval(() => {
      ...
      if (p >= 100) {
        ...
        toast('识别完成，低置信字段已标黄，请进入 Step2 校对');
      }
    }, 380);
  };
  const reOcr = () => {
  ...
  const projLine = () => {
    ...
    if (pmode === 'draft') return <>项目：新建草稿「{f.pjname.trim()}」→ <b>PRJ-DRAFT-001</b>（签约后自动转已立项）</>;
    ...
    setConfirm({
      ...
      cb: () => {
        ...
        const newContract: Parameters<typeof addContract>[0] = {
          ...
          amt: f.amt, execAmt: f.amt, status: '审批中', recvPct: 0, recv: 0,
  ...
  const resetAll = () => {
    setSrc('manual'); setRoute([0, 3]); setStep(0);
  ...
  /* ---------- 渲染：步骤条 ---------- */
  const stepNames = ['选择创建来源', '来源处理', 'OCR 校对', '补充与提交'];
  ...
  return (
    <div className="nc-wiz">
      ...
      <div className="nc-steps">
        {[0, 1, 2, 3].map((i) => {
          const st = step === i ? 'is-cur' : !route.includes(i) ? 'is-skip' : i < step ? 'is-done' : '';
          const mark = st === 'is-done' ? <Ico n="check" size={13} /> : st === 'is-skip' ? '—' : i + 1;
          return (
            <div key={i} className={`nc-step ${st}`} onClick={() => { if (st === 'is-done') goStep(i); }} {...pressProps(() => { if (st === 'is-done') goStep(i); })}>
              <span className="nc-step-dot">{mark}</span>
              <span className="nc-step-label">Step{i} · {stepNames[i]}</span>
            </div>
          );
        })}
      </div>
      ...
      {step === 0 && (
        <Card>
          ...
          <div className="nc-pgfoot" style={{ marginTop: 12 }}>
            选择来源后进入分叉：OCR → Step2 校对；模板 → 预览 + 变量；复制 → 差异 + 确认；手工 → 直入 Step3。<b>创建方式选定后在 Step3 锁定只读。</b>
          </div>
        </Card>
      )}
      ...
      {/* ============ Step1 来源处理 ============ */}
      {step === 1 && src === 'ocr' && (
        <Card hd="Step1 · 上传合同并识别">
          <Banner tone="info">支持 PDF / JPG / PNG ≤50MB；异步识别 ≤10s；<b>重新识别 ≤3 次</b>（按单据计次，在 Step2 操作）。</Banner>
          {!ocrRun && !ocrDone && (
          ...
          {(ocrRun || ocrDone) && (
            <>
              ...
              <div className="nc-ops" style={{ marginTop: 12 }}>
                <Btn kind="primary" disabled={!ocrDone} title={ocrDone ? undefined : '等待 OCR 识别完成（识别中请稍候）'} onClick={() => goStep(2)}>进入 Step2 校对 →</Btn>
      ...
      {step === 1 && (src === 'std' || src === 'ent') && (
        <Card hd={`Step1 · ${src === 'std' ? '选择标准模板' : '选择企业模板'}`}>
          {src === 'ent' && (
          ...
          <div className="nc-varbox">
            {([['a', '发包方名称'], ['b', '承包方名称'], ['p', '项目名称'], ['m', '金额']] as [keyof typeof tplVars, string][]).map(([k, t]) => (
              <div className="nc-varpair" key={k}>
                ...
                <input className="nc-input" placeholder={k === 'm' ? '同步至 Step3 合同金额' : k === 'a' ? '同步至 Step3 相对方' : `请输入${t}`}
      ...
      {step === 1 && src === 'copy' && (
        <Card hd="Step1 · 复制历史合同（差异预览 · 逐项确认带入）">
          ...
          <div className="nc-cell-sub">仅可选 已签约 / 履约中 / 已结项；排序：本客户优先 → 同类型</div>
          {!copySel
      ...
      {/* ============ Step2 OCR 校对（左图右字段） ============ */}
      {step === 2 && src === 'ocr' && (
        <Card hd="Step2 · OCR 校对（左图右字段）" extra={<Btn size="sm" disabled={tries <= 0} title={tries > 0 ? `每日重新识别上限 3 次，剩余 ${tries} 次` : '重新识别次数已用完（每日上限 3 次），请人工校对'} onClick={reOcr}>重新识别（剩 {tries} 次）</Btn>}>
          ...
          <div className="nc-ocr-grid">
            ...
            <div>
              ...
              <div className="nc-cell-sub" style={{ marginTop: 12 }}>确认后 OCR 值带入 Step3（可继续修改）</div>
            </div>
          </div>
        </Card>
      )}
        ...
        <>
          ...
          <Card hd="基本信息">
            <div className="nc-form-grid">
              ...
              <Field label="创建方式"><input className="nc-input" value={`${src ? SRC_META[src].n : '—'}（Step0 锁定只读）`} readOnly /></Field>
      ...
      <div className="nc-pgfoot">
        编号规则：HT 销售 / WB 维护保养 / CG 采购 / FK 分包 / KJ 框架（提交时生成 · 不可改）｜审批路由：合同类型 × 金额档矩阵（自动计算）｜OCR 重识别 ≤3 次｜附件：单份 ≤50MB · 每类 ≤5 份 · DWG/DXF 仅「其他」类｜A-04：提交时检测「选择已有」项目未归并收支（硬拦截）｜草稿项目 PRJ-DRAFT-xxx 签约后自动转已立项
      </div>
      ...
      {/* 通用确认弹窗 */}
      <Modal
        open={!!confirm} onClose={() => setConfirm(null)} width={520}
        title={confirm?.title ?? ''}
