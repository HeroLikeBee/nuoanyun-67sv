...
import { QUOTE_CATS, UNITS, MATERIALS, CUSTOMERS, OPPS, fmt, fmtWan, approveLevel, quoteTrigger, TODAY } from '../components/data';
...
export default function QuoteEditPage({ go, role, nav }: { go: (p: string) => void; role: string; nav?: number }) {
  ...
  return (
    <>
      ...
      <Card>
        <div className="nc-addbar">
          <div className="nc-ops">
            <Btn kind="primary" size="sm" onClick={() => setAddOpen(true)}><Ico n="building" size={16} /> 从材料库添加</Btn>
      ...
      {/* ===== 成本参考价三源 ===== */}
      <Drawer open={!!refOpen} onClose={() => setRefOpen(null)} width={560} title="成本参考价 · 三源摘要" sub={refOpen && `${refOpen.name}（当前成本价 ${fmt(refOpen.cost)}）`}>
        {refOpen && (
          <>
            <KvGrid cols={1} rows={[
              { k: '① 最近采购价', v: <>{fmt(refOpen.cost * 0.96)} · 2026-08-12 · 供应商 云南滇安消防设备有限公司</> },
              { k: '② 供应商框架价', v: <>{fmt(refOpen.cost * 1.02)} · 有效至 2026-12-31 · 框架协议 FK20260115-0002</> },
              { k: '③ 历史同类合同价', v: <>{fmt(refOpen.cost * 1.05)} · 2026-05 · 曲靖一院消防升级合同</> },
              { k: '建议价', v: <><b className="num">{fmt(refOpen.cost / (1 - 0.2))}</b> · 建议价 = 成本价 ÷ (1 − 红线 20%)</> },
            ]} />
            <div className="nc-listhint">
              <span>参考线<Tip w={340} text={<><Ico n="warning" size={13} /> 报价价低于成本价 → 锁死不可提交（须理由 + 特批）；偏离建议价 &gt;10% 橙字、&gt;20% 红字。</>} /></span>
            </div>
            <div className="nc-sec-title">成本参考价改动留痕</div>
            <div className="nc-form-grid">
              <Field label="修改为" note="改动将记录谁 / 何时 / 旧值→新值"><input className="nc-input" type="number" defaultValue={refOpen.cost} /></Field>
              <Field label="修改理由" req><input className="nc-input" placeholder="例：供应商调价，按框架价更新" /></Field>
            </div>
          </>
        )}
      </Drawer>
